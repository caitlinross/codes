...
/* SUMMARY:
 *
 * Orchestrator-driven twin of tests/modelnet-test.c. It runs the identical
 * simulation -- the same server LP exchanging requests/acks over a model-net
 * NIC, the same CLI options, the same lp-io output -- but drives the setup
 * sequence through codes::orchestrator instead of the hand-written
 * configuration_load / model_net_register / codes_mapping_setup /
 * model_net_configure boilerplate.
 *
 * The server LP below is a deliberate copy of modelnet-test.c's (ported to
 * compile as C++: the malloc results are cast). It exists to demonstrate the
 * orchestrator against a real model; keeping it identical is what makes the two
 * executables equivalent. The executable-equivalence ctest guards the copy: if
 * the original LP changes and this one does not, the two binaries diverge and
 * the test fails.
 */
...
extern "C" const tw_lptype* svr_get_lp_type();
...
/* Orchestrator net-id callback: receives the network ids produced by
 * model_net_configure() and hands this model the (single) one it sends to. */
...
int main(int argc, char** argv) {
    ...
    /* Drive config load, LP-type registration, model_net_register,
     * codes_mapping_setup, model_net_configure and net-id distribution through
     * the orchestrator instead of writing them out by hand. */
    ...
    if (lp_io_dir[0]) {
        ...
        if (lp_io_prepare(lp_io_dir, flags, &handle, MPI_COMM_CODES) < 0) {
            return (-1);
        }
    }
    ...
    if (do_lp_io && lp_io_flush(handle, MPI_COMM_CODES) < 0) {
        return (-1);
    }
...
static void handle_kickoff_event(svr_state* ns, svr_msg* m, tw_lp* lp) {
    ...
    if ((net_id == DRAGONFLY || net_id == SLIMFLY || net_id == EXPRESS_MESH) &&
        (lp->gid % lps_per_rep == (tw_lpid)(num_servers_per_rep - 1)))
...
static void handle_req_event(svr_state* ns, svr_msg* m, tw_lp* lp) {
    ...
    if ((net_id == DRAGONFLY || net_id == SLIMFLY || net_id == EXPRESS_MESH) &&
        (m->src % lps_per_rep == (tw_lpid)(num_servers_per_rep - 1)))
        opt_offset = num_servers_per_rep +
                     num_routers_per_rep; /* optional offset due to dragonfly mapping */
